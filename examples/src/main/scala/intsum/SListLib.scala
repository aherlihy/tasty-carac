...
def int_sum_baseline(): Unit = {
  var sum: Long = 0
  var line: Option[String] = None

  while ( {
    line = Option(StdIn.readLine()); line.isDefined
  }) {
    sum += line.get.toInt
//    println(s"sum=$sum")
  }
...
def int_sum_binary(): Unit = {
  val dataInputStream = new DataInputStream(new BufferedInputStream(System.in))
  var sum: Long = 0
  val buffer = new Array[Byte](4)

  while (dataInputStream.read(buffer) != -1) {
    val wrappedBuffer = ByteBuffer.wrap(buffer).order(ByteOrder.LITTLE_ENDIAN)
    sum += wrappedBuffer.getInt
    dataInputStream.read() // Read and ignore the newline character
  }
...
def int_sum_binary_BE(): Unit = {
  ...
  while ({ bytesRead = input.read(buffer); bytesRead != -1 }) {
    if (bytesRead == 4) {
      val value = ByteBuffer.wrap(buffer).order(ByteOrder.BIG_ENDIAN).getInt
      sum += value
    }
  }
  println(sum)
